...
function anthropicRequestToOpenAI(payload, { defaultModel } = {}) {
  ...
  out.model = selectAnthropicTargetModel(payload.model, defaultModel);
...
function selectAnthropicTargetModel(requestedModel, defaultModel) {
  const requested =
  ...
  // Claude Code sends Anthropic model ids (for example "claude-sonnet-...").
  // ToCodex is reached through an OpenAI-compatible chat.completions endpoint,
  // so a configured TOCODEX_DEFAULT_MODEL should replace those ids.
  if (requested && !(fallback && isAnthropicModelId(requested))) return requested;
  return fallback || requested;
}

function isAnthropicModelId(model) {
  return /^claude(?:-|$)/iu.test(model) || /^anthropic[/:]/iu.test(model);
}
